        ...
        $rootNode
            ->children()
                ->booleanNode('return_form_pdf_enabled')
                    ...
                    ->defaultFalse()
                ->end()
                ->booleanNode('allow_unpaid_withdrawal')
                    ->info('When true (default), an unpaid not-yet-shipped order can be withdrawn by the customer: the Sylius order is cancelled and the return resolves directly to the terminal "withdrawn" state with no admin step. When false, an unpaid order cannot start a withdrawal.')
                    ->defaultTrue()
                ->end()
                ->arrayNode('resources')
                    ->addDefaultsIfNotSet()
